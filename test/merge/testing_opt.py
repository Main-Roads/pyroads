...
res_legacy = merge.on_slk_intervals(
    segments,
    data,
    join_left=["road", "cwy"],
    ...
    column_actions=[
        merge.Action(
            "measure_a", rename="longest", aggregation=merge.Aggregation.KeepLongest()
        ),
        merge.Action(
            "measure_a", rename="mean", aggregation=merge.Aggregation.Average()
        ),
        merge.Action(
            "measure_a",
            rename="lenw_mean",
            aggregation=merge.Aggregation.LengthWeightedAverage(),
        ),
        merge.Action(
            "measure_a",
            rename="lenw_prc75",
            aggregation=merge.Aggregation.LengthWeightedPercentile(0.75),
        ),
        merge.Action(
...
legacy_time = time.time() - start_time_legacy
print("Legacy merge took --- %s seconds ---" % (legacy_time))
print(res_legacy)
print(res_legacy.dtypes)

start_time_optimized = time.time()
res_optimized = merge.on_slk_intervals_optimized(
    segments,
    data,
    join_left=["road", "cwy"],
    from_to=("slk_from", "slk_to"),
...
print("Optimized merge took --- %s seconds ---" % (optimized_time))
print(res_optimized)
print(res_optimized.dtypes)
print(50 * "#")
print(res_legacy.equals(res_optimized))
print(
